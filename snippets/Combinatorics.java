...
public class Combinatorics {
    ...
    // ========== nCr - Combination ==========
    // Correct implementation: Multiply incrementally and divide by (i+1) each step
    // This ensures we always have integer results (never remainder) because C(n,r) is always an integer
    static long nCr(int n, int r) {
        ...
        // Calculate: (n * (n-1) * ... * (n-r+1)) / (1 * 2 * ... * r)
        // We do this incrementally to avoid overflow
        for (int i = 0; i < r; i++) {
            result *= (n - i);
            result /= (i + 1);
        }
    ...
    // Use this for large numbers or when MOD is required
